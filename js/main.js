;(function () {
		...
		if ( project.length > 0 ) {	
			...
			project.waypoint( function( direction ) {
				...
				if( direction === 'down' && !$(this.element).hasClass('animated') ) {
					...
					setTimeout(function() {
						project.find('.to-animate').each(function( k ) {
							var el = $(this);
					...
					setTimeout(function() {
						project.find('.to-animate-3').each(function( k ) {
							var el = $(this);
		...
		if ( contact.length > 0 ) {	
			...
			contact.waypoint( function( direction ) {
				...
				if( direction === 'down' && !$(this.element).hasClass('animated') ) {
					...
					var sec = contact.find('.to-animate').length,
						sec = parseInt((sec * 200) + 400);
	...
	document.getElementById('contact-form').addEventListener('submit', function(event) {
		...
		xhr.open('POST', 'https://script.google.com/macros/s/AKfycbzCP04VBsEf98-ovmzoRCXde-H9DV_gVcJ78Ezjt5lCQnwXJ_dFIUjz-o-JoNHxo7GS/exec', true);
		xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
		xhr.onload = function() {
		  if (xhr.status === 200) {
			// Show success message
			document.getElementById('contact-form').innerHTML = '<p>Message sent!</p>';
		  } else {
			// Show error message
			document.getElementById('contact-form').innerHTML = '<p>Something went wrong. Please try again later.</p>';
		  }
		};
		xhr.send(new FormData(event.target)); // Send form data using AJAX
	  });
